...
(function() {
    (function() {
                ...
                if (info) {
                    try {
                        ...
                        if (!applyToProperty(comp) && !applyToLayer(comp)) {
                            ...
                            effect(BLENDING_MODE_PARAM).setValue(1);
                        }
                    } catch (e) {
                        alert(e);
                    } finally {
        ...
        function addSolid(comp, width, height) {
            return Atarabi.API.invoke(HOOK_SOLID, ADD_SOLID, [ comp, width, height ], function(comp, width, height) {
                return comp.layers.addSolid([ 1, 1, 1 ], "Solid (".concat(width, "x").concat(height, ")"), width, height, 1);
            });
        }
        ...
        function applyToLayer(comp, file) {
            ...
            if (file) {
                setImageFromFile(effect, file, 2);
            } else {
                setImageFromClipboard(effect, 2);
            }
            return true;
        }
        var MOVIE_EXTS = {
