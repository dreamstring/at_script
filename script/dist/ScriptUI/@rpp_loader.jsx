...
(function() {
    (function(global) {
        ...
        var RPPLoader = function() {
            ...
            RPPLoader.prototype.load = function(file, contents) {
                ...
                if (isArray(tracks)) {
                    for (var _i = 0, tracks_1 = tracks; _i < tracks_1.length; _i++) {
                        ...
                        var isbus = map(track.properties["ISBUS"][0], function(val) {
                            return +val;
                        });
                        ...
                        prevIsbus = isbus;
                        var trackMute = some(mutes, function(mute) {
                            return mute;
                        });
                ...
                if (root.properties["VIDEO_CONFIG"]) {
                    this.size = map(root.properties["VIDEO_CONFIG"][0], function(val) {
                        return +val;
                    });
                }
                ...
                if (root.properties["TEMPO"]) {
                    this.tempo = map(root.properties["TEMPO"][0], function(val) {
                        return +val;
                    });
                }
                        ...
                        for (var _c = 0, PTs_1 = PTs; _c < PTs_1.length; _c++) {
                            ...
                            if (PT[3] && parseInt(PT[3], 10) > 0) {
                                ...
                                this.tempos.push({
                                    ...
                                    shape: shape,
                                    signature: {
                                        num: signature & 0xFFFF,
                                        den: signature >> 16 & 0xFFFF
                                    }
                                });
                            } else {
                                this.tempos.push({
            ...
            RPPLoader.prototype.execute = function(_a, isChild) {
                ...
                if (target === Target.Auto) {
                    comp = getCompItem(function(item) {
                        return item.comment === _this.file.absoluteURI;
                    });
                    if (!comp) {
                ...
                this.createLayers(comp, analyzer, {
                    sort: sort,
            ...
            RPPLoader.prototype.importItem = function(path, placeholder) {
                ...
                return getFootageItem(function(item) {
                    return item.file && item.file.absoluteURI === file.absoluteURI;
                }) || app.project.importFile(new ImportOptions(file));
            };
            RPPLoader.prototype.importPlaceholder = function(file, path) {
                var items = filter(this.items, function(item) {
                    return item.getFilePath() === path;
                });
                var duration = reduce(items, function(duration, item) {
                    return Math.max(duration, item.getEstimatedDuration());
                }, 0);
        ...
        var builder = new Atarabi.UI.Builder(global instanceof Panel ? global : "palette", SCRIPT_NAME, {
        ...
        }).addGroupEnd().addButton(Param.Load, undefined, undefined, function(ui, emitter) {
            ui.alignment = [ "fill", "top" ];
            ui.helpTip = "Load";
            ui.onClick = function() {
                emitter.notify(Event.Load);
            ...
            ui.alignment = [ "fill", "top" ];
        }).addPanelEnd().addPanel(Param.ExecutePanel, "Execute", undefined, function(ui) {
            ui.alignment = [ "fill", "top" ];
            ui.orientation = "column";
            ui.preferredSize[0] = 230;
            ui.spacing = 5;
            ...
            ui.preferredSize[0] = 100;
            ui.selection = 0;
        }).addGroupEnd().addGroup(Param.OptionsGroup, undefined, function(ui) {
            ui.alignment = [ "fill", "top" ];
            ui.orientation = "row";
            ui.spacing = ui.margins = 1;
            ...
            ui.preferredSize[0] = 100;
        }).addGroupEnd().addButton(Param.Execute, undefined, undefined, function(ui, emitter) {
            ui.alignment = [ "fill", "top" ];
            ui.helpTip = "Execute";
            ui.onClick = function() {
                if (!loader.valid()) {
