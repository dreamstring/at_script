...
(() => {
	...
	class Mat3x3 {
		...
		clone(): Mat3x3 {
			return new Mat3x3([...this.mat[0]], [...this.mat[1]], [...this.mat[2]]);
		}
	...
	type ParserSucess<T> = [success: true, position: number, value: T];
	...
	const ch: ChFunc =
		<T extends string>(c: T) =>
		input =>
			is((v): v is typeof c => v === c)(input);
	...
	const makeSequence =
		<T, U>(parser: Parser<T>, sep: Parser<U>): Parser<T[]> =>
		([text, pos]) => {
			const value: T[] = [];
	...
	// common
	const coordinateSequence = makeSequence(
		map(coordinateOfPath, v => parseFloat(v)),
		commaWsp
	);

	const vector2dSequence = makeSequence(
		map(coordinatePair, arr => A.map(arr, parseFloat) as Vector2),
		commaWsp
	);
	...
	// main
	const ellipticalArcArgument = map(
		cat([nonNegativeNumber, opt(commaWsp), nonNegativeNumber, opt(commaWsp), number, commaWsp, flag, opt(commaWsp), flag, opt(commaWsp), coordinatePair]),
		arr => {
	...
	const drawToCommand = or<PathCommand>([
		closePath,
		lineTo,
		horizontalLineto,
		verticalLineto,
	...
	const color: Parser<Atarabi.Color | Atarabi.ColorA> = or([
		...
		map(cat([ch('#'), hexDigit, hexDigit, hexDigit, hexDigit, hexDigit, hexDigit]), arr => {
			return [
				parseInt(`${arr[1]}${arr[2]}`, 16) / 255,
				parseInt(`${arr[3]}${arr[4]}`, 16) / 255,
				parseInt(`${arr[5]}${arr[6]}`, 16) / 255
			] satisfies Atarabi.Color;
		}),
		// #FFF
		map(cat([ch('#'), hexDigit, hexDigit, hexDigit]), arr => {
			return [
				parseInt(`${arr[1]}${arr[1]}`, 16) / 255,
				parseInt(`${arr[2]}${arr[2]}`, 16) / 255,
				parseInt(`${arr[3]}${arr[3]}`, 16) / 255
			] satisfies Atarabi.Color;
		}),
		// rgb(100%, 100%, 100%)
	...
	const paint: Parser<Paint> = or<Paint>([color, str('none'), str('transparent'), reg(/^url\(.*\)/)]);
	...
	const coordinatePairOfPoints = or([
		map(cat([number, commaWsp, number]), arr => {
			return [parseFloat(arr[0]), parseFloat(arr[2])] satisfies Vector2;
		}),
		map(cat([number, negativeNumber]), arr => {
			return [parseFloat(arr[0]), parseFloat(arr[1])] satisfies Vector2;
		})
	]);
	...
	const opacity: Parser<Opacity> = or<Opacity>([map(number, v => M.clamp(parseFloat(v), 0, 1)), str('inherit')]);

	type FillRule = 'nonzero' | 'evenodd' | 'inherit';
	const fillRule: Parser<FillRule> = or<FillRule>([str('inherit'), str('evenodd'), str('inherit')]);
	...
	const strokeWidth: Parser<StrokeWidth> = or<StrokeWidth>([length, str('inherit')]);

	type StrokeLineCap = 'butt' | 'round' | 'square' | 'inherit';
	const strokeLineCap: Parser<StrokeLineCap> = or<StrokeLineCap>([str('butt'), str('round'), str('square'), str('inherit')]);
	...
	const strokeLineJoin: Parser<StrokeLineJoin> = or<StrokeLineJoin>([str('miter'), str('round'), str('bevel'), str('inherit')]);
	...
	const strokeMiterLimit: Parser<StrokeMiterLimit> = or<StrokeMiterLimit>([map(number, v => parseFloat(v)), str('inherit')]);
	...
	const strokeDashArray: Parser<StrokeDashArray> = or<StrokeDashArray>([dashArray, str('none'), str('inherit')]);

	type StrokeDashOffset = number | 'inherit';
	const strokeDashOffset: Parser<StrokeDashOffset> = or<StrokeDashOffset>([length, str('inherit')]);
	...
	const STYLE_ATTRIBUTES: (keyof Style)[] = [
		'fill',
		'fill-opacity',
		'fill-rule',
		'stroke',
	...
	const ellipticalArcToBezier = (
		...
		x2: number,
		y2: number
	): {c1: Vector2; c2: Vector2; en1: Vector2; en2: Vector2}[] => {
		fa = +!!fa;
		fs = +!!fs;
		let rX = Math.abs(rx);
		let rY = Math.abs(ry);
	...
	const applyGradient = (group: GradientFillGroup | GradientStrokeGroup, gradient: Gradient) => {
		if (gradient.type === 'linear') {
			...
			group
				...
				.startPoint([gradient.cx, gradient.cy])
				.endPoint([gradient.cx + gradient.r, gradient.cy]);
	...
	const applyTransformByMatrix = (group: ShapeGroup, m: Mat3x3) => {
		...
		group
			.transform()
			.scale()
			.setValue([100 * trans.scale[0], 100 * trans.scale[1]]);
		if (trans.skew[0] !== 0) {
	...
	var Atarabi: Atarabi = $.global.Atarabi;
	Atarabi.SVG = (Atarabi.SVG || {}) as any;
	...
	const main = (file: File) => {
		...
		const comp =
			app.project.activeItem instanceof CompItem
				...
				: app.project.items.addComp(fileName, Math.ceil(svg.viewBox[2]), Math.ceil(svg.viewBox[3]), 1, 10, 30);
